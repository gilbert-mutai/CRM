...
def notify_veeam(action, company_name, user):
    ...
    if action == "add":
        message = f"(Veeam): A new Veeam record for {company_name} has been added by {user_name}"
    elif action == "update":
        message = f"(Veeam): Veeam record for {company_name} has been modified by {user_name}"
    elif action == "delete":
        message = f"(Veeam): Veeam record for {company_name} has been deleted by {user_name}"
    else:
        message = f"(Veeam):Veeam record for {company_name} was changed by {user_name}"
